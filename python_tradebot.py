...
def get_balance(symbol):
    try:
        ...
        logging.error(
            "Found error. status: {}, error code: {}, error message: {}".format(error.status_code,
                                                                                error.error_code,
                                                                                error.error_message
                                                                                )
        )
...
def indicator_cal():
    ...
    kline_data['k'], kline_data['d'] = talib.STOCH(ohlcv['high'], ohlcv['low'], ohlcv['close'], fastk_period=9, slowk_period=3, slowd_period=3)
        ...
        if not trade_flag:
            ...
            if new_time - old_time >= 60:
                ...
                if direction != '':
                    ...
                    print('開單日期: {} 開單價格: {} 開單數量: {} 開單手續費: {}'.format(open_time, open_price, trade_num, open_fee))
                old_time = new_time
        elif trade_flag:
            new_time = time.time()
            if new_time - old_time >= 60:
                ...
                if direction == 'BUY':
                    if now_price - open_price >120 * dup_profit:
                        if now_direction == '' or now_direction == 'SELL':
                            ...
                            closs_price = get_order(SYMBOL, orderId)
                            close_fee = closs_price / 20 * trade_num * 0.0002
                    ...
                    if dup_time >= 2:
                        if now_price - open_price > 50 * dup_profit:
                            if now_direction == '' or now_direction == 'SELL':
                                ...
                                closs_price = get_order(SYMBOL, orderId)
                                close_fee = closs_price/20 * trade_num * 0.0002
                                profit = (closs_price - open_price)*trade_num - open_fee -close_fee
                                open_fee2 = ''
                                dup_time = 0
                                direction = ''
                                open_time = ''
                    ...
                    if open_price - now_price > 120 * dup_profit:
                        if now_direction == '' or now_direction == 'BUY':
                            ...
                            profit = (open_price - close_price) * trade_num - open_fee - close_fee
                            dup_time = 0
                            direction = ''
                            open_time = ''
                            open_price = 0.0
